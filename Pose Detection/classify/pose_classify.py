...
pose_sample_rpi_path = os.path.join(scripts_dir,  "raspberry_pi")
...
pd.DataFrame(class_names).to_csv("/cnvrg/class_names.csv")
...
# validation accuracy.
checkpoint_path = "/cnvrg/weights.best.hdf5"
...
eval_metrics.to_csv("/cnvrg/eval_metrics_1.csv")
...
for pose in poses_1:
    ...
    print('drow')
    for file1 in files:
        shutil.move(os.path.join(source, file1), '/cnvrg/'+pose)
        print(file1)
# source = pose_path
# files=os.listdir(source)
# for file1 in files:
...
pd.DataFrame(y_test).to_csv("/cnvrg/actual_values_test.csv")
pd.DataFrame(y_pred_label).to_csv("/cnvrg/predicted_labels.csv")
pd.DataFrame(y_pred).to_csv("/cnvrg/predicted_values.csv")
df_test_1.to_csv("/cnvrg/test_data_frame.csv")
cm1 = pd.DataFrame(cm, columns=class_names)

cm1.index = list(class_names)
cm1.to_csv("/cnvrg/cm.csv")

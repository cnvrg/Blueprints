...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if opt.optimizer == 'Adam':
        optimizer = Adam(g0, lr=hyp['lr0'], betas=(
            hyp['momentum'], 0.999))  # adjust beta1 to momentum
    elif opt.optimizer == 'AdamW':
        optimizer = AdamW(g0, lr=hyp['lr0'], betas=(
            hyp['momentum'], 0.999))  # adjust beta1 to momentum
    else:
        optimizer = SGD(g0, lr=hyp['lr0'],
    ...
    if RANK in [-1, 0]:
        ...
        #################################
        traintest_loader = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                             hyp=hyp, cache=None if noval else opt.cache, rect=True, rank=-1,
                                             workers=workers, pad=0.5,
                                             prefix=colorstr('traintest: '))[0]
        #################################
        if not resume:
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            if RANK in [-1, 0]:
                ...
                callbacks.run('on_train_batch_end', ni, model,
                              imgs, targets, paths, plots, opt.sync_bn)
            # end batch ------------------------------------------------------------------------------------------------
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = val.run("Trainresults.csv",
                                            data_dict,
                                            batch_size=batch_size // WORLD_SIZE * 2,
                                            imgsz=imgsz,
                                            model=attempt_load(
                    ...
                    #####################
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val.run("Valresults.csv",
                                            data_dict,
                                            batch_size=batch_size // WORLD_SIZE * 2,
                                            imgsz=imgsz,
                                            model=attempt_load(
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str,
                        default='yolov5s.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT /
                        'data/custom_train.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--class_names', type=str,
                        default='', help='Names of the classes')

    opt = parser.parse_known_args()[0] if known else parser.parse_args()

    #########################
    classes = str(opt.class_names)
    if(classes.endswith('.csv')):
        classes = str(list(pd.read_csv(classes)['classes']))
    else:
        classes = str(classes.split(','))

    number = str(len(classes.split(',')))

    train_path = "train: "+"../../input/object_detection_recreate/images/train/ "
    val_path = "val: "+"../../input/object_detection_recreate/images/val/ "
    test_path = "test: "+"../../input/object_detection_recreate/images/test/ "

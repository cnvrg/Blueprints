...
parser.add_argument('--reg_pu', action='store', dest='reg_pu', default=0.06, required=True,
...
for nmf1 in range(len(users)):
    ...
    recommend = recommend[~recommend['item_id'].isin(
        train_whole.loc[(train_whole['user_id'] == nmf1)]['item_id'].to_list() +
        test_whole.loc[(test_whole['user_id'] == nmf1)]['item_id'].to_list()
        )]
    ...
    rec_rel_cnt_nmf = user3_movie_pred.rating.loc[
        (user3_movie_pred.rating >= threshold * max_v) & (user3_movie_pred.score >= threshold * user_max2)].count()
    rec_rel_cnt_nmf_k = user3_movie_pred[:K].rating.loc[
        (user3_movie_pred.rating >= threshold * max_v) & (user3_movie_pred.score >= threshold * user_max2)].count()

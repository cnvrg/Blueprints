...
for als1 in range(len(users)):
    ...
    recommend = recommend[~recommend['item_id'].isin(
        train_whole.loc[(train_whole['user_id'] == als1)]['item_id'].to_list() +
        test_whole.loc[(test_whole['user_id'] == als1)]['item_id'].to_list()
        )]
    ...
    rec_rel_cnt_als = user2_movie_pred.rating.loc[
        (user2_movie_pred.rating >= threshold * max_v) & (user2_movie_pred.score >= threshold * user_max1)].count()
    rec_rel_cnt_als_k = user2_movie_pred[:K].rating.loc[
        (user2_movie_pred.rating >= threshold * max_v) & (user2_movie_pred.score >= threshold * user_max1)].count()

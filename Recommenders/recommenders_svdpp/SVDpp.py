...
parser.add_argument('--num_of_steps_1', action='store', dest='num_of_steps_1', default=1, required=True,
...
parser.add_argument('--learning_rate', action='store', dest='learning_rate', default=0.007, required=True,
                    help="""learning rate for SGD""")
parser.add_argument('--std_dev_1', action='store', dest='std_dev_1', default=0.05, required=True,
                    help="""standard deviation for svd""")
...
svdpp = SVDpp(n_factors=n_factors_1, verbose=False, reg_all=reg_coef_1, n_epochs=n_iters_1, init_std_dev=std1,
              lr_all=alpha, random_state=10)
...
for user in range(n_users):
    for item in range(n_items):
        predic[user, item] = svdpp.predict(uid=user, iid=item).est
...
for svg1 in range(len(users)):
    ...
    scores_3.columns = scores_3.columns.str.replace(' ', '')
    ...
    recommend = recommend[~recommend['item_id'].isin(
        train_whole.loc[(train_whole['user_id'] == svg1)]['item_id'].to_list() +
        test_whole.loc[(test_whole['user_id'] == svg1)]['item_id'].to_list()
        )]
    ...
    rec_rel_cnt_svg = user3_movie_pred.rating.loc[
        (user3_movie_pred.rating >= threshold * max_v) & (user3_movie_pred.score >= threshold * user_max2)].count()
    rec_rel_cnt_svg_k = user3_movie_pred[:K].rating.loc[
        (user3_movie_pred.rating >= threshold * max_v) & (user3_movie_pred.score >= threshold * user_max2)].count()
...
metrics_file_name = 'SVDpp_eval_metrics_file.csv'
dataset_pred_name = 'SVDpp_user1_movie_pred_whole.csv'
eval_metrics_file = 'SVDpp_eval_metrics_file.csv'
...
e.log_param("SVDpp_ram", psutil.virtual_memory()[3] / (1024 * 1024 * 1024))
e.log_param("SVDpp_time", toc - tic)

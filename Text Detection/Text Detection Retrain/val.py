...
def run(locationcsv,
    ...
    datadict['mAP@.5'].append(round(map50, 3))
    datadict['mAP@.5:.95'].append(round(map, 3))
    
    if(locationcsv!="null" and not training):
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        ...
        for i, c in enumerate(ap_class):
            ...
            datadict['mAP@.5'].append(round(ap50[i], 3))
            datadict['mAP@.5:.95'].append(round(ap[i], 3))
        df = pd.DataFrame(datadict)
        df.to_csv("../../cnvrg/"+locationcsv, index=False)
        # Save csv results
    # Print speeds
